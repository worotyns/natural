...
Deno.test('permission', () => {
  ...
  assertEquals(myflags, new Map<unknown, number>([
    ['payments.read', 1],
    ['payments.create', 2],
    ['payments.update', 4],
    ['payments.delete', 8],
  ...
  assertEquals(combine(myflags.get('payments.create')!, myflags.get('payments.read')!), role, 'combine works');
  ...
  const myRole = combine(myflags.get('payments.create')!, myflags.get('payments.delete')!);
  assertEquals(
    or(myRole, myflags.get('payments.admin')!, and(myRole, myflags.get('payments.create')!, myflags.get('payments.delete')!)),
    true,
    'or works'
  )
});