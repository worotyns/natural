...
Deno.test("organism.atom", () => {
  ...
  assertEquals(
    myAtom.identity,
    ["item"],
    "should not combine identifiers for atom",
  );
...
Deno.test("organism.molecule", () => {
  ...
  assertEquals(
    myMol.identity,
    ["dev", "users", "matias@eu.pl"],
    "should combine identifiers for molecule",
  );
});
...
Deno.test("organism.cell", async () => {
  ...
  assertEquals(results.identity.length, 3);
  assertEquals(
    results.identity.slice(0, 2),
    ["dev", "durable_counter"],
