...
type AtomCollection = (AnyAtom & BaseAtomHelpers<unknown>)[] ;
...
type PrimitiveList = Primitive[];
type Primitive = string | boolean | number | Date | PrimitiveObject | PrimitiveList;

// atom must have identity
// atom must have value
// atom must have a version stamp
export type Atom<ValueType, ValueTypeKind extends PrimitiveValue = PrimitiveValue> = {
...
type BaseAtomHelpers<V> = {
  ...
  serialize(references?: SerializedAtomWithReferences): SerializedAtomWithReferences;
  // basic primitive for updates
  mutate(value: V): void;
}

// base logic for persistence and archiving
type BaseRuntimeHelpers = {
...
export function atom<V, K extends PrimitiveValue, H extends BaseAtomHelpers<V>>(value: V, valueKind: K, identity: IdentityInstance, version: Versionstamp, helpers: H): Atom<V, K> & H {
  return {
    ...helpers,
    identity,
    kind: PrimitiveKind.Atom,
...
export type StringAtom = Atom<string, PrimitiveValue.String> & StringAtomHelpers;

// string atom with validation, guards and other helpers
export function string(value: string, identity: IdentityInstance): StringAtom {
  return atom<string, PrimitiveValue.String, StringAtomHelpers>(value, PrimitiveValue.String, identity, "", {
...
// number atom helpers
export type NumberAtom = Atom<number, PrimitiveValue.Number> & NumberAtomHelpers & BaseAtomHelpers<number>;
...
export function number(value: number, identity: IdentityInstance): NumberAtom {
  return atom<number, PrimitiveValue.Number, NumberAtomHelpers>(value, PrimitiveValue.Number, identity, "", {
...
export type BooleanAtom = Atom<boolean, PrimitiveValue.Boolean> & BooleanAtomHelpers & BaseAtomHelpers<boolean>;
...
export function boolean(value: boolean, identity: IdentityInstance): BooleanAtom {
  return atom<boolean, PrimitiveValue.Boolean, BooleanAtomHelpers>(value, PrimitiveValue.Boolean, identity, "", {
...
export function date(value: Date, identity: IdentityInstance): DateAtom {
  return atom<Date, PrimitiveValue.Date, DateAtomHelpers>(value, PrimitiveValue.Date, identity, "", {
...
export type ObjectAtom = Atom<PrimitiveObject, PrimitiveValue.Object> & ObjectAtomHelpers;

// object atom with validation, guards and other helpers
export function object(value: PrimitiveObject, identity: IdentityInstance): ObjectAtom {
  return atom<PrimitiveObject, PrimitiveValue.Object, ObjectAtomHelpers>(value, PrimitiveValue.Object, identity, "", {
...
// boolean atom helpers
export type ListAtom = Atom<PrimitiveList, PrimitiveValue.List> & ListAtomHelpers & BaseAtomHelpers<PrimitiveList>;
...
export function list(value: PrimitiveList, identity: IdentityInstance): ListAtom {
  return atom<PrimitiveList, PrimitiveValue.List, ListAtomHelpers>(value, PrimitiveValue.List, identity, "", {
...
export type CollectionAtom = Atom<AtomCollection, PrimitiveValue.Collection> & CollectionAtomHelpers & BaseAtomHelpers<AtomCollection>;
...
export function collection(value: AtomCollection, identity: IdentityInstance): CollectionAtom {
  return atom<AtomCollection, PrimitiveValue.Collection, CollectionAtomHelpers>(value, PrimitiveValue.Collection, identity, "", {
    mutate(this: CollectionAtom, newValue: AtomCollection) {
      this.value = newValue;
    },
    serialize(this: CollectionAtom, references: SerializedAtomWithReferences = {}) {

      for (const atom of this.value) {
        const reference = atom.identity.serialize();
        if (reference in references) {
          throw new RuntimeError('Cannot serialize atoms, with same identity: ' + reference);
        }
...
// map atom helpers
export type MapAtom = Atom<AtomMap, PrimitiveValue.Map> & MapAtomHelpers & BaseAtomHelpers<AtomMap>;
...
// atom collection, guards and other helpers
export function map(value: AtomMap, identity: IdentityInstance): MapAtom {
  return atom<AtomMap, PrimitiveValue.Map, MapAtomHelpers>(value, PrimitiveValue.Map, identity, "", {
